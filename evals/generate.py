...
def eval(model_path, model, eval_data, tokenizer, output_file, device, use_vllm=False):
    ...
    if "tofu" in model_path.lower():
        text_column = "question" # paraphrased_question
        # text_column = "paraphrased_question" # paraphrased_question
        labels_column = "answer" # paraphrased_answer
        # labels_column = "paraphrased_answer" # paraphrased_answer
    elif "harry" in model_path.lower():
    ...
    for result in results:
        ...
        rouge_scores = scorer.score(generated_response, ground_truth)
        rouge1_recall.append(rouge_scores['rouge1'].recall)
        rougeL_recall.append(rouge_scores['rougeL'].recall)
    rouge_recall = {'rouge1_recall': rouge1_recall, 'rougeL_recall': rougeL_recall}
    mean_rouge_recall = {k: np.mean(v) for k, v in rouge_recall.items()}
    print(f"==================Paraphrased Mean ROUGE Recall: {mean_rouge_recall}======================")
    ...
    if use_vllm:
        ...
        if use_4bit:
            ...
            llm = LLM(
                ...
                load_format="bitsandbytes",      # 必须添加此行，以匹配量化方法
