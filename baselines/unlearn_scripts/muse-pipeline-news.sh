...
CUDA_ID=0
################################模型训练##################################
# forget_data_path="/data1/muse_bench/data/news/raw/forget.txt"
# retain_data_path="/data1/muse_bench/data/news/raw/retain1.txt"

# idonknow_file_path="../../dataset/idontknow.txt"

# model_family=muse-news-llama2-7b
# model_path="/data1/relearn/paper_models/muse-news-3e-4-2048-llama2-7b-lora16-target-epoch20"
# lr=1e-4
# num_epochs=5
# ds_config="../config/ds_z0_config.json"
# # "ga_gdr" "ga_klr" "npo_gdr" "npo_klr" "npo" "AGO"
# loss_types=("sim_npo_gdr" "npo") 
# max_length=2048
# ema_decay=0.9
...
base_model_path="/data1/relearn/paper_models/muse-news-3e-4-2048-llama2-7b-lora16-target-epoch20"
...
for adapter_dir in "$memory_dir"/*/; do
  ...
  if [[ "$adapter_name" != "muse-news-llama2-7b_npo_2048_1e-4" ]]; then
  ...
  if [[ "$adapter_name" == *llama2* ]]  && [[ "$adapter_name" != *-full ]]; then
    for checkpoint_dir in "$adapter_dir"*/; do
      if [[ "$checkpoint_dir" == *checkpoint-40* ]]; then
