...
def finetune(cfg):
    ...
    if "tofu" in data_file.lower() or "knowundo" in data_file.lower() or "harry" in data_file.lower():
        print("using qa dataset..")
        dataset = QADataset(
            data_file,
            tokenizer=tokenizer,
            max_len=max_length,
            model_cfg=model_cfg # 这里之前没加model_cfg
        )
    else:
    ...
    # model = AutoModelForCausalLM.from_pretrained(model_id, use_flash_attention_2=model_cfg["flash_attention2"]=="true", dtype=torch.bfloat16, trust_remote_code = True)
