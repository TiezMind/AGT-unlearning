...
class SURE(Trainer):
    """Custom Trainer for Unlearning with Neuron-Level Saliency Map 基于神经元级显著性图的遗忘定制训练器"""
            ...
            for name, param in model.named_parameters():
                if param.grad is not None:
                    ...
                    if grad.dim() > 1:
                        ...
                        # if "mlp.gate_proj.lora_A" in name:
                        #     print(f"Gradient norms for {name}: {grad_norms_per_neuron}")
                    else:
                        # For 1D parameters (e.g., biases)
                        grad_norms_per_neuron = grad.abs().cpu().numpy()

                    # print("grad_norms_per_neuron.shape:",grad_norms_per_neuron.shape)
                    for idx, grad_norm in enumerate(grad_norms_per_neuron):
                        neuron_name = f"{name}.{idx}"
                        neuron_grad_norms[neuron_name] = grad_norm
            # end_time = time.time()
            # print(f"Neuron-wise Gradient Norm Computation Time: {end_time - start_time:.2f} seconds")
