...
def unlearn(cfg):
    wandb.init(
        project="unlearning-llama-MUSE",  # 项目名
        notes=cfg.loss_type,  # 核心：添加备注
        name=cfg.run_name  # 可选：自定义实验名
    )
    ...
    model = AutoModelForCausalLM.from_pretrained(cfg.model_path, config=config, dtype=torch.bfloat16, trust_remote_code = True)
    ...
    config = LoraConfig(
        ...
        target_modules=find_all_linear_names(model), #{'down_proj', 'up_proj', 'gate_proj','q_proj', 'k_proj', 'v_proj', 'o_proj'}
    ...
    training_args = transformers.TrainingArguments(
            ...
            bf16=True,
            bf16_full_eval=True,
            logging_steps=min(1,max_steps//20),
            logging_dir=f'{cfg.save_dir}/logs',
            output_dir=cfg.save_dir,
            optim="paged_adamw_32bit",
            save_strategy="steps" if cfg.save_model and (not cfg.eval_only) else "no",
            save_steps=steps_per_epoch2 if loss_type == "npo" else steps_per_epoch,
    ...
    # if "sure" in cfg.loss_type or cfg.loss_type == "relearn_klr_gdr":
