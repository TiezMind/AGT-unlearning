...
def compute_undial_loss(model, ref_model, inputs, beta):
    ...
    pre_softmax = shift_teacher_logits - mask * beta # 直接硬编码将logits减去1合理吗
...
class IterativeUnlearner(Trainer):
    ...
    def __init__(self, *args,
        ...
        self.delta = kwargs.pop("delta", 0.0)  # Only relevant when 'sim_npo' in self.loss_type

        super().__init__(*args, **kwargs)
        if self.ref_model is not None:
            assert 'po' in self.loss_type or 'kl' in self.loss_type
            print("===========================使用deepspeed加载ref_model===========================")
    ...
    def compute_loss(self, model, x, return_outputs=False, num_items_in_batch=None):
        ...
        elif self.loss_type == 'sim_npo_gdr':
            outputs_f = model(
                x_f['input_ids'],
                labels=x_f['labels'] if 'labels' in x_f else x_f['input_ids'].clone(),
                attention_mask=x_f['attention_mask'] if 'attention_mask' in x_f else torch.ones_like(x_f['input_ids'], dtype=torch.bool)
        ...
        elif self.loss_type == 'sim_npo_klr':
            outputs_f = model(
                x_f['input_ids'],
                labels=x_f['labels'] if 'labels' in x_f else x_f['input_ids'].clone(),
                attention_mask=x_f['attention_mask'] if 'attention_mask' in x_f else torch.ones_like(x_f['input_ids'], dtype=torch.bool)
        ...
        elif self.loss_type == 'UNDIAL':
            loss_f, forget_outputs = compute_undial_loss(
                model, self.ref_model, x_f, self.beta
            )
